...
// share notes modal
export default function ShareNotesModal({ onClose, notes, batches }) {
    const [selectedBatches, setSelectedBatches] = useState([]);
    const [selectedStudents, setSelectedStudents] = useState([]);
    const [sharing, setSharing] = useState(false);

    // Get list of already assigned emails from the notes prop
    const alreadyAssigned = useMemo(() => notes?.sharedWith || [], [notes]);

    // Toggle Batch Logic
    const toggleBatch = (batchCode) => {
        const batch = batches.find(b => b.batchCode === batchCode);

        // Filter out students who already have access to these notes
        ...
        if (selectedBatches.includes(batchCode)) {
            ...
            if (eligibleStudents.length === 0) {
                return toast.error("All students in this batch already have access to these notes.");
            }
    ...
    const handleShare = async () => {
        ...
        setSharing(true);
        const tid = toast.loading("Distributing study notes...");

        try {
            const res = await fetch("https://2dsr6yh6rc.execute-api.ap-south-1.amazonaws.com/mentor/notes", {
                method: "POST",
            ...
            toast.success("Notes successfully shared with students!", { id: tid });
            onClose();
        } catch (err) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
            <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="p-8 border-b border-slate-50 flex justify-between items-center">
                    <div>
                        <h3 className="text-xl font-black text-slate-800">Share Study Notes</h3>
                        <p className="text-xs text-indigo-600 font-bold uppercase tracking-widest mt-1">{notes?.title}</p>
                    </div>
                ...
                <div className="p-8 overflow-y-auto space-y-8 custom-scrollbar">
                    ...
                    <section>
                        ...
                        <div className="flex flex-wrap gap-3">
                            {batches?.map(batch => {
                                // Visual indicator if a batch is already fully assigned access
